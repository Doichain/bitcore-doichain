 ...
 * @param offChainUtxos
 * @param network
 * @returns {Promise<{tx: *, doichainEntry: void, validatorAddress: string, changeAddress: string, utxos: *, validatorPublicKeyData: *}>}
 */
export const createDOIRequestTransaction = async (email, ourWallet, offChainUtxos, network) => {

    const validatorPublicKeyData = await getValidatorPublicKey(email)
    console.log('validatorPublicKeyData',validatorPublicKeyData)
    const doichainEntry = await createDoichainEntry(validatorPublicKeyData.key,email,ourWallet.privateKey,ourWallet.senderEmail)
    console.log('doichainEntry',doichainEntry)

    const ourAddress = bitcore.getAddressOfPublicKey(ourWallet.publicKey, network).toString()
    console.log('ourAddress',ourAddress)
    const utxos = await getUTXOs4DoiRequest(ourAddress,offChainUtxos)

    const validatorAddress = bitcore.getAddressOfPublicKey(validatorPublicKeyData.key,network).toString()
    ...
    const txSignedSerialized = await bitcore.createRawDoichainTX(
        ...
        bitcore.constants.VALIDATOR_FEE.satoshis //0.01 for DOI storage, 0.01 DOI for reward for validator, 0.01 revokation reserved
    )
 ...
 * @returns {Promise<*>}
 */
export const getValidatorPublicKey = (email) => {
    const parts = email.split("@");
    const domain = parts[parts.length-1];
    let our_validatorPublicKeyData = getPublicKey(domain)
...
export const createDoichainEntry = async (validatorPublicKey,recipientEmail, ourPrivateKey, senderEmail ) => {
    //TODO can be removed
    let our_doichainEntry =  await bitcore.createDoichainEntry(ourPrivateKey, validatorPublicKey, senderEmail, recipientEmail)
...
export const encryptEmailVerification = (validatorPublicKey,senderEmail,address) => {
    console.log("validatorPublicKey",validatorPublicKey)
    console.log("senderEmail",senderEmail)
    console.log("address",address)
    const publicKey = Buffer.from(validatorPublicKey, 'hex');
    const message = Buffer.from(JSON.stringify({sender_mail:senderEmail,address: address}));
...
export const encryptTemplate = (validatorPublicKeyData, email,ourWallet) => {
   ...
   const our_encryptedTemplateData =  bitcore.encryptMessage(
